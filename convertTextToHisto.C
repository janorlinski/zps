...
TH1I getHistoFromTxtFile (TString fileName) {
	...
	while (getline(myStr, value_temp)) {
		
		//if (verbatim) std::cout << value_temp << "\n";
		value.push_back(stoi(value_temp));
...
vector<TH1I> getHistosFromTxtFileList (TString fileList) {
	...
	for (int i=0; i<NHistos; i++) {
		...
		chi2NDF1->SetBinContent(i+1, chi2NDF1vec[i]/(i+1));
		chi2NDF2->SetBinContent(i+1, chi2NDF2vec[i]/(i+1));
...
int convertTextToHisto (TString listName) {
	...
	//TString listName = "lista_20umRun47.list";
